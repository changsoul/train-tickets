(function() {
    ...
    jQuery.extend({
        ...
        showRandCodeBefore: function() {
            $(".captchaButton").each(function(k, j) {
                var l = $(j);
                var n = l.data("touclick-name");
                var m = '<div class="touclick" name="touclick-' + $.getRandCodeName(n) + '"><div class="touclick-wrapper"><div class="touclick-bgimg touclick-reload touclick-reload-normal"></div><div class="touclick-bgimg touclick-arrow"></div><div class="touclick-img-par touclick-bgimg"  style=" width: 293px; height: 190px;"><img class="touclick-image" alt="" src=""/></div></div></div>';
                l.prepend(m)
            });
            for (var e = 0; e < targetdiv.length; e++) {
                var f = $.getRandCodeName(targetelement[e]);
                $("#" + targetdiv[e]).after('<input type="hidden" name="' + $.getRandCodeName("") + '" data-targetdiv="' + targetdiv[e] + '" id="' + f + '" /><li id="error_msg' + targetdiv[e] + '" class="error" style="display:none" ></li>')
            }
        ...
        showRandCode: function() {
            ...
            TouClick.ready(function() {
                ...
                f.show();
                $("<a href='../gonggao/yzmsysm.html' target='_blank' style='display:block;float:right;margin-right:11px;font-size:12px;line-height:25px;'>验证码如何使用？</a>").insertBefore(d);
                m.parent().css({
                    height: "340px"
                }).parent().next().css({
                    height: "360px"
                ...
                $('<span style="float:left;padding-right:2px;display:block;height:30px;width:65px;text-align:right;">验证码：</span>').insertBefore(g)
            })
        },
