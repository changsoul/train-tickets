...
public class MyButton extends JButton {
	...
	protected void init() {
		...
		try {
			...
			if("buy".equals(type)) {
				icon = bgBtn.getSubimage(0, 250, 5, 30);
			} else if("price".equals(type)) {
				icon = bgBtn.getSubimage(0, 0, 5, 30);
			} else {
				icon = bgBtn.getSubimage(0, 220, 5, 30);
			}
			
			Icon ic1 = new ImageIcon(ImageUtil.resizeImage(icon, 72, 30));
